<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="nickname字段" prop="nickname" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateWechatGroupFunc(scope.row)">变更</el-button>
...
const getTableData = async() => {
  const table = await getWechatGroupList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteWechatGroupByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateWechatGroupFunc = async(row) => {
    const res = await findWechatGroup({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteWechatGroupFunc = async (row) => {
    const res = await deleteWechatGroup({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
