  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateWechatGroupUserFunc(scope.row)">变更</el-button>
...
import {
  ...
  deleteWechatGroupUser,
  deleteWechatGroupUserByIds,
  updateWechatGroupUser,
  findWechatGroupUser,
  getWechatGroupUserList
...
const getTableData = async() => {
  const table = await getWechatGroupUserList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteWechatGroupUserByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateWechatGroupUserFunc = async(row) => {
    const res = await findWechatGroupUser({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteWechatGroupUserFunc = async (row) => {
    const res = await deleteWechatGroupUser({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
